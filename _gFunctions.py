...
'''
    ...
    return (result.alternatives[0].transcript.encode('utf-8'), result.alternatives[0].confidence)
...
def upload_file(file_stream, filename, content_type):
    ...
    bucket = client.bucket("registrazioni_lunhge")
...
def main2(audio_url):
    ...
    transcript, offsets, confidence = long_audio("gs://registrazioni_lunhge/"+audio_url)
    outf.write("\n|||\n**********Niente***********")
    outf.write("Transcript:\n{}\n\nConfidence: {}".format(transcript, confidence))
    transcript, offsets, confidence = long_audio("gs://registrazioni_lunhge/"+audio_url, phrases_words)
    outf.write("\n|||\n**********Parole separate***********")
    outf.write("Transcript:\n{}\n\nConfidence: {}".format(transcript, confidence))
    transcript, offsets, confidence = long_audio("gs://registrazioni_lunhge/"+audio_url, phrases_rows)
    outf.write("\n|||\n**********Righe separate***********")
    outf.write("Transcript:\n{}\n\nConfidence: {}".format(transcript, confidence))
    outf.close()
